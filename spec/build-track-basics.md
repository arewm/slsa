...
| [Build L4] | Complete dependency provenance enforced by the build platform | Unknown or unrecorded build dependencies
...
## Build L4: Complete dependency provenance

<dl class="as-table">
<dt>Summary<dd>

The build platform has technical controls ensuring that every artifact consumed
during the build is recorded in the provenance. This enables consumers to
recursively verify the entire build-time supply chain.

<dt>Intended for<dd>

Software producers and build platforms wanting to provide strong supply chain
integrity guarantees that extend recursively to all build-time dependencies,
not just the top-level build process.
...
-   Build platform:
    -   Implement technical controls in the trusted control plane to ensure
        that `resolvedDependencies` in the provenance is complete — all
        artifacts consumed during the build are accounted for.

<dt>Benefits<dd>

All of [Build L3], plus:

-   Enables consumers to [recursively apply SLSA][verification] to all
    build-time dependencies, closing gaps that exist when dependency recording
    is only best-effort.

-   Prevents unrecorded dependencies from being used as a covert injection
    vector — any dependency that influences the build must appear in the
    provenance.

-   Provides the foundation for policy-based verification of the full
